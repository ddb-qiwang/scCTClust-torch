...
class CITECluster(ModelBase):
    def __init__(self, cfg):
        ...
        self.fusion = get_fusion_module(cfg.fusion_cfg, self.mvae.output_sizes)
        # Define clustering module
        self.ddc = DDC(input_dim=self.fusion.output_size, cfg=cfg.cm_config)
        # Define projector
        self.projector = nn.Identity()
        # Define loss config
        self.loss = Loss(cfg.loss_cfg)
        # Define optimizer config
        self.optimizer = Optimizer(cfg.opt_cfg, self.parameters())
        # Initialize weights.
